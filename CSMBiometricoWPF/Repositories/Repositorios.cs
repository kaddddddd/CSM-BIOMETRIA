...
namespace CSMBiometricoWPF.Repositories
{
    ...
    public class RegistroIngresoRepository
    {
        ...
        /// <summary>
        /// Inserta un registro de inasistencia justificada para un día sin registro previo.
        /// </summary>
        public void InsertarJustificado(RegistroIngreso reg, EstadoIngreso estado, string observacion, string? franja)
        {
            using var conn = ConexionDB.ObtenerConexion();
            using var cmd  = new MySqlCommand(@"
                INSERT INTO registros_ingreso
                    (id_estudiante, id_sede, fecha_ingreso, hora_ingreso, estado_ingreso, puntaje_biometrico, observaciones, nombre_franja)
                VALUES (@est, @sede, @fecha, '00:00:00', @estado, 0, @obs, @franja)", conn);
            cmd.Parameters.AddWithValue("@est",    reg.IdEstudiante);
            cmd.Parameters.AddWithValue("@sede",   reg.IdSede);
            cmd.Parameters.AddWithValue("@fecha",  reg.FechaIngreso.ToString("yyyy-MM-dd"));
            cmd.Parameters.AddWithValue("@estado", estado.ToString());
            cmd.Parameters.AddWithValue("@obs",    string.IsNullOrWhiteSpace(observacion) ? DBNull.Value : (object)observacion.Trim());
            cmd.Parameters.AddWithValue("@franja", string.IsNullOrWhiteSpace(franja)      ? DBNull.Value : (object)franja.Trim());
            cmd.ExecuteNonQuery();
        }
